...
if (inActions && !npmToken && !oidcReady) {
  error("No npm publish credentials — cannot publish to npm", [
    ...
    "  1. https://www.npmjs.com/package/@crvouga/mockingbird-service-sqlite → Settings → Trusted Publisher",
...
if (inActions && !npmToken && oidcReady) {
  console.log(
    "release-preflight: NPM_TOKEN unset; using GitHub OIDC (npm Trusted Publishing must be configured for @crvouga/mockingbird-service-sqlite).",
  );
}
