...
import { document, PharmetikaAPI, supportedOperationIds } from "./src/index.js"

const params = fcParameters(process.env)
const MOCK_HOST = "mock.pharmetika.local"
const now = () => 1_700_000_000_000
const auth = { "x-pmk-authentication-token": "parity-token" }

describe("PharmetikaAPI", () => {
  test(
    "self-parity: independent instances agree on every random walk and conform to the spec",
    async () => {
      const reference = new PharmetikaAPI({ now })
      ...
      // Every operation, including patient create, submit and cancel, is reached by the walks.
    ...
    async () => {
      await fc.assert(
        fc.asyncProperty(fc.integer(), async (seed) => {
          const reference = new PharmetikaAPI({ now })
          const faulty = () => {
            const api = new PharmetikaAPI({ now })
            return {
              fetch: async (request: Request) => {
                const response = await api.fetch(request)
                if (!new URL(request.url).pathname.endsWith("/clinic_list")) return response
                const body = (await response.json()) as { data: { identifier: string }[] }
                body.data[0] = { ...body.data[0], identifier: "diverged" } as never
