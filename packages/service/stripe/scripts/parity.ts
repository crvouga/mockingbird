...
const credentials = await loadCredentials(
  { provider: "stripe", fields: { secret_key: "MOCKINGBIRD_STRIPE_SECRET_KEY" } },
  { env: process.env, readTokenFile },
)
const secretKey = credentials.values.secret_key
...
await parity({
  ...
  cleanup: async ({ table, real }) => {
    const del = (path: string) =>
      real.fetch(new Request(`${real.baseUrl}${path}`, { method: "DELETE", headers: authHeaders }))
