...
import { document, EasyPostAPI, supportedOperationIds } from "./src/index.js"

const params = fcParameters(process.env)
const MOCK_HOST = "mock.easypost.local"
const now = () => 1_700_000_000_000
const auth = { authorization: `Basic ${btoa("EZTKparity:")}` }

describe("EasyPostAPI", () => {
  test(
    "self-parity: independent instances agree on every random walk and conform to the spec",
    async () => {
      const reference = new EasyPostAPI({ now })
    ...
    async () => {
      await fc.assert(
        fc.asyncProperty(fc.integer(), async (seed) => {
          const reference = new EasyPostAPI({ now })
          const faulty = () => {
            const api = new EasyPostAPI({ now })
            return {
              fetch: async (request: Request) => {
                const response = await api.fetch(request)
                if (request.method !== "POST" || response.status !== 201) return response
                const body = (await response.json()) as Record<string, unknown>
                return Response.json({ ...body, status_detail: "diverged" }, { status: 201 })
              },
            }
          }
