...
import { DailyAPI, document, supportedOperationIds } from "./src/index.js"

const params = fcParameters(process.env)
const MOCK_HOST = "mock.daily.local"
const now = () => 1_700_000_000_000

const auth = () => ({ authorization: "Bearer daily-parity-key" })

describe("DailyAPI", () => {
  test(
    "self-parity: independent instances agree on every random walk and conform to the spec",
    async () => {
      const reference = new DailyAPI({ now })
    ...
    async () => {
      await fc.assert(
        fc.asyncProperty(fc.integer(), async (seed) => {
          const reference = new DailyAPI({ now })
          const faulty = () => {
            const api = new DailyAPI({ now })
            return {
              fetch: async (request: Request) => {
                const response = await api.fetch(request)
                if (response.status !== 200) return response
                const body = (await response.json()) as { privacy: string }
                return Response.json(
                  { ...body, privacy: body.privacy === "public" ? "private" : "public" },
                  { status: 200 },
                )
              },
            }
