...
import { document, PortalAgentAPI, supportedOperationIds } from "./src/index.js"

const params = fcParameters(process.env)
const MOCK_HOST = "mock.portal-agent.local"
const now = () => 1_700_000_000_000
const auth = { authorization: "Bearer parity-api-key" }

describe("PortalAgentAPI", () => {
  test(
    "self-parity: independent instances agree on every random walk and conform to the spec",
    async () => {
      const reference = new PortalAgentAPI({ now })
    ...
    async () => {
      await fc.assert(
        fc.asyncProperty(fc.integer(), async (seed) => {
          const reference = new PortalAgentAPI({ now })
          const faulty = () => {
            const api = new PortalAgentAPI({ now })
            return {
              fetch: async (request: Request) => {
                const response = await api.fetch(request)
                if (response.status !== 202) return response
                const body = (await response.json()) as Record<string, unknown>
                return Response.json({ ...body, message: "diverged" }, { status: 202 })
              },
            }
          }
