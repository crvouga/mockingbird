// @ts-nocheck -- vendored upstream source, type-checked by Medplum under its own compiler settings (see README.md).
...
import type { FhirRequest, FhirRouteHandler, FhirRouteMetadata, FhirRouter, RestInteraction } from './fhirrouter.js';
import type { FhirRepository } from './repo.js';
import type { HttpMethod, RouteResult } from './urlrouter.js';
...
interface ProcessingState {
  /** Index into `bundleInfo.ordering` of the next entry to run. */
  position: number;
...
export class BatchProcessor {
   ...
   * @param position - The index into `bundleInfo.ordering` of the next entry to run.
  ...
  /**
   * @returns The index into `bundleInfo.ordering` of the next entry to run. This is the
  ...
  private parsePatchBody(entry: BundleEntry): any {
    ...
    if (patchResource?.resourceType === 'Binary') {
      ...
      body = JSON.parse(decodeBase64Utf8(patchResource.data));
