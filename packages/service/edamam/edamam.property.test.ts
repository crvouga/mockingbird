...
import { document, EdamamAPI, supportedOperationIds } from "./src/index.js"

const params = fcParameters(process.env)
const MOCK_HOST = "mock.edamam.local"
const now = () => 1_700_000_000_000
const auth = { accept: "application/json" }

describe("EdamamAPI", () => {
  test(
    "self-parity: independent instances agree on every random walk and conform to the spec",
    async () => {
      const reference = new EdamamAPI({ now })
