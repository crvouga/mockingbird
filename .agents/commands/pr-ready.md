---
name: pr-ready
description: Take the current branch to a merge-ready PR (commit, publish, sync with main, resolve conflicts, open the PR, fix every failing check — CI and third-party like GitGuardian) and stop there; never merges.
---

# /pr-ready

<!-- Canonical file: .agents/commands/pr-ready.md. Every agent harness symlinks here
     (bun run agents:sync) — edit this file, never a link. -->

Drive the current branch from local work-in-progress to a PR that is **ready to merge into `main`**.
All mechanical git/GitHub work is done by `bun scripts/pr-ready.ts`; your job is the judgment: commit message,
conflict resolution, PR title/body, and root-cause fixes for every failing check.

The job is done when the PR is ready: the branch is pushed, synced with `main`, not a draft, and
**every** check on the PR has passed. **Never merge it** — not with the script, `gh pr merge`, auto-merge,
or the GitHub UI. Merging to `main` publishes, and that call belongs to the user.
...
```
bun run pr:ready advance --timeout 1800
```

It syncs both the remote head and `origin/main`, publishes, creates a PR if needed using the
non-merge commit subjects, marks a draft ready, waits for every check, and runs the `ready` gate. It
stops with one JSON object at the first blocker. Fix the reported conflict or check and rerun. Pass
`--title` and `--body-file` to control a new PR's text. Confirm `ready: true`; pending checks are not
...
- A check that failed for a transient reason (runner crash, network blip, an app that timed out) —
  ...
  ```
  bun run pr:ready rerun --name "<check name>"
  ```
...
2. **A test fixture or false positive** (a made-up value that only a mock accepts):
   ...
   - If it **is pushed**, the history still holds the value, so resolve the incident in
     ...
     ```
     bun run pr:ready guardian ignore --incident <id> --reason test_credential
     bun run pr:ready rerun --name "GitGuardian Security Checks"
     bun run pr:ready checks
     ```
...
Reviews, approvals, and unresolved threads are not part of the ready gate. To read them anyway:

```
bun run pr:ready comments
```

The JSON lists unresolved review threads and recent general comments. Reply to a thread with
`bun run pr:ready reply --thread <id> --body-file <file>`, and
`bun run pr:ready resolve --thread <id>` after its concern is addressed. Neither is required
before `ready`.

## 7. Verify ready

When `checks` exits 0, run the ready gate. It verifies, without merging:
...
```
bun run pr:ready ready
```

- exit 0 (`ready: true`) → done; go to step 8.
- exit 4 (`pending`) → checks or GitHub's mergeability are still settling; run `checks`, then `ready`
  again.
- exit 1 (`step: "ready-gate"`) → each entry in `blockers` says what to do: commit/publish, `sync`
...
Report exactly: branch, PR number and URL, base branch, the PR `state` (must be `OPEN`, not a draft),
every check name with its bucket, and the required contexts. Say the PR is ready for the user to
merge. Success requires `ready: true` — never report success on a partial, queued, or filtered run.
...
- Never merge the PR, enable auto-merge, or approve it — stop at ready.
- Never disable or bypass a check. A check that ran is required.
